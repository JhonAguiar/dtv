    ...
    <div class="col-lg-12">
        <div class="ibox float-e-margins">
            ...
            <div class="ibox-content ibox-form">
                ...
                <form name="upgrade" id="upgrade" action="{{ url('Argentina/provisioning/update') }}" method="POST" enctype="multipart/form-data">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}" readonly="readonly">
                    <p class="text-justify">{{ trans('messages.000077') }}</p>
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        function get_profiles(argument) {
            var options='';
            $("#spinnerLoader").show();
            $.post("{{ url('imsi/technology')}}",{
                "_token" : "{{ csrf_token() }}",
        ...
        $(document).ready(function() { 
			...
			$("form#upgrade").submit(function(evt){	 
				evt.preventDefault();
				...
				swal({
				    ...
				    if (isConfirm) {
						...
						$("#spinnerLoader").show();
						$("#data_return").html('&nbsp;');
						var formData = new FormData($('form#upgrade')[0]);
						$.ajax({
						    url : $('form#upgrade').attr('action'),// la URL para la petición
            ...
            $( "#technology" ).change(function() {
                get_profiles($(this).val());
            });

            get_profiles($("#technology").val());
