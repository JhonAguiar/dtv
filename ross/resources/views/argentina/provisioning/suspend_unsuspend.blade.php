    ...
    <div class="col-lg-12">
        <div class="ibox float-e-margins">
            ...
            <div class="ibox-content ibox-form">
                ...
                <form name="suspend-unsuspend" id="suspend-unsuspend" action="{{ url('Argentina/provisioning/suspend-unsuspend') }}" method="POST" enctype="multipart/form-data">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}" readonly="readonly">
                    <p class="text-justify">
	                    {{ trans('messages.000070') }}<br>
	                    {{ trans('messages.000071') }}<br>
	                    {{ trans('messages.000075') }}
	                </p>
					...
					<div class="row">
					    ...
					    <div class="col-sm-3">
					        <div class="form-group">
					            <label for="method" class="control-label">{{ trans('messages.000018') }}</label>
					    ...
					    <div class="col-sm-3">
					        <div class="form-group">
					            ...
					            <button type="submit" id="btnEnviar" class="btn btn-success btn-block">{{ trans('messages.buttons.06') }}</button>
					        </div>
					    </div>
					</div>
				</form>
...
@section('scripts')
    ...
    <script type="text/javascript">
        $(document).ready(function() { 
			...
			$("form#suspend-unsuspend").submit(function(evt){	 
				evt.preventDefault();
				...
				swal({
				    ...
				    if (isConfirm) {
						...
						$("#spinnerLoader").show();
						$("#data_return").html('&nbsp;');
						var formData = new FormData($('form#suspend-unsuspend')[0]);
						$.ajax({
						    url : $('form#suspend-unsuspend').attr('action'),// la URL para la petición
